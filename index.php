...
while($resp["code"]==200){
    ...
    elseif($action == "request"){
        ...
        $resp = $auth->getResp();
        if($resp["code"]!=200)break;
        if(!isset($_REQUEST["vin"])){$resp = setErrorResponse(500,"No required field 'vin'");break;}
        $vin = strtoupper($_REQUEST["vin"]);
        if(!VIN::validate($vin)){$resp = setErrorResponse(500,"Incorrect field value 'vin'");break;}
        $result = [];
